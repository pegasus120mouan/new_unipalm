@php
    ...
    $montantFinancement = (float) ($montantFinancement ?? 0);
    $useFinancement = ($source ?? '') === 'montants' && $montantFinancement > 0;
    $plafondPaiement = $useFinancement
        ? min((float) ($resteAPayer ?? 0), $montantFinancement)
        : (float) ($resteAPayer ?? $usine->reste_a_payer ?? 0);
@endphp

<div class="modal fade" id="{{ $modalId }}" tabindex="-1" aria-labelledby="{{ $modalId }}Label" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="{{ $paymentAction ?? route('usines.amounts.payment', $usine) }}">
                @csrf
                @if (! empty($redirectTo))
                ...
                @if ($useFinancement)
                    <input type="hidden" name="mode_paiement" value="Financement">
                @endif
                ...
                <div class="modal-body">
                    ...
                    <div class="alert alert-light border mb-3">
                        ...
                        @if ($useFinancement)
                            <div class="mt-1">
                                <span class="text-muted">Financement disponible :</span>
                                <strong style="color: #6f42c1;">
                                    {{ number_format($montantFinancement, 0, '', ' ') }} FCFA
                                </strong>
                            </div>
                            <div class="small text-muted mt-2">
                                Ce paiement sera <strong>déduit du financement</strong>
                                (plafond : {{ number_format($plafondPaiement, 0, '', ' ') }} FCFA).
                            </div>
                        @else
                            <div class="small text-muted mt-1">
                                Le montant saisi sera crédité (ravitaillé) sur la banque sélectionnée.
                            </div>
                        @endif
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success"
                        @disabled(! $useFinancement && $banques->isEmpty())>
